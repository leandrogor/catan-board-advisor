...
@Component({
  ...
  template: `
    <div
      class="flex items-center justify-between gap-2 px-3 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom,0))] bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm border-t border-slate-200 dark:border-slate-700"
    >
      <div class="flex items-center gap-1.5 shrink-0">
        <button
          class="flex items-center gap-1 px-2.5 py-1.5 text-xs sm:text-sm font-medium rounded-lg transition-colors
        ...
        >
          <span>←</span> <span class="hidden min-[380px]:inline">{{ i18n.t().undo }}</span>
        </button>
        ...
        >
          <span class="hidden min-[380px]:inline">{{ i18n.t().redo }}</span> <span>→</span>
        </button>
      </div>
      ...
      <button
        class="flex items-center gap-1 px-2.5 py-1.5 text-xs sm:text-sm font-medium rounded-lg transition-colors
               ...
               hover:bg-indigo-200 dark:hover:bg-indigo-800/50 shrink-0"
      ...
      @if (store.appPhase() !== 'setup') {
        <span
          class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-medium ml-1 truncate text-right max-w-30 min-[380px]:max-w-none"
        >
          {{ settlementText() }}
        </span>
      }
